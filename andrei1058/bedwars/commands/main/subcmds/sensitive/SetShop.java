                ...
                if (ss.getCm().getYml().get("Team") != null) {
                    ...
                    for (String team : ss.getCm().getYml().getConfigurationSection("Team").getKeys(false)) {
                        p.spigot().sendMessage(Misc.msgHoverClick("§6 ▪ " + TeamColor.getChatColor(ss.getCm().getYml().getString("Team." + team + ".Color")) + team + "§7 (click to set)",
                                "§7Set shop for " + TeamColor.getChatColor(ss.getCm().getYml().getString("Team." + team + ".Color")) + team, "/" + mainCmd + " setShop " + team, ClickEvent.Action.RUN_COMMAND));
                    }
                }
            } else {
                ss.getCm().saveArenaLoc("Team." + args[0] + ".Shop", p.getLocation());
                p.sendMessage("§6 ▪ §7Shop set for: " + TeamColor.getChatColor(ss.getCm().getYml().getString("Team." + args[0] + ".Color")) + args[0]);
            }
        }
        return true;
    }
