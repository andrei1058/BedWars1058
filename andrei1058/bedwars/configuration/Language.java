    ...
    public static void setupLang(Language lbj) {
        ...
        switch (lbj.iso) {
            default:
                ...
                yml.addDefault(Messages.PLAYER_DIE_DEBUG_REGULAR, "{PlayerColor}{PlayerName} &7was killed by {KillerColor}{KillerTeamName}'s &7Debug!");
                yml.addDefault(Messages.PLAYER_DIE_DEBUG_FINAL_KILL, "{PlayerColor}{PlayerName} &7was killed by {KillerColor}{KillerTeamName}'s &7Debug! &b&lFINAL KILL!");
                ...
                yml.addDefault(Messages.COMMAND_PARTY_INVITE_SENT_TARGET_RECEIVE_MSG, "{prefix}&7{player} &eis not online!");
    ...
    public static void setupCustomStatsMessages() {
        for (Language l : getLanguages()) {
            ...
            for (String item : Main.config.getYml().getConfigurationSection("statsGUI").getKeys(false)) {
                ...
                l.yml.addDefault(Messages.PLAYER_STATS_GUI_PATH + "." + item + ".lore", Arrays.asList("lore not set"));
            }
            l.save();
        }
    }
