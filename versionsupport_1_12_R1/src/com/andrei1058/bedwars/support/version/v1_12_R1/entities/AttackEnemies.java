package com.andrei1058.bedwars.support.version.v1_12_R1.entities;
...
@SuppressWarnings({"unchecked", "SimplifiableConditionalExpression", "ConstantConditions", "WeakerAccess", "Guava"})
public class AttackEnemies<T extends EntityLiving> extends PathfinderGoalTarget {
    ...
    private boolean isMember(EntityLiving t0) {
        return (t0 instanceof EntityPlayer && bedWarsTeam.isMember(Bukkit.getPlayer(t0.getName()))) ? true :
                (Main.nms.isDespawnable(t0.getBukkitEntity()) && Main.nms.getDespawnablesList().get(t0.getBukkitEntity().getUniqueId()).getTeam() == bedWarsTeam);
    }
