...
public class UpgradeTier {
    ...
    public UpgradeTier(String parentName, String name, ItemStack displayItem, int cost, Material currency) {
        ...
        this.name = name;
        BedWars.getAPI().getLanguageService().saveIfNotExists(Messages.UPGRADES_UPGRADE_TIER_ITEM_NAME.replace("{name}", parentName.replace("upgrade-", "")).replace("{tier}", name), "&cName not set");
        BedWars.getAPI().getLanguageService().saveIfNotExists(Messages.UPGRADES_UPGRADE_TIER_ITEM_LORE.replace("{name}", parentName.replace("upgrade-", "")).replace("{tier}", name), Collections.singletonList("&cLore not set"));
        this.cost = cost;
