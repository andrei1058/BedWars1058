...
public class Internal implements Party {
        ...
        for (Party p : new ArrayList<>(getParites())) {
            if (p.owner == member) {
                ...
                for (Player mem : p.members) {
                    mem.sendMessage(SupportPAPI.getSupportPAPI().replace(mem, Language.getMsg(mem, Messages.COMMAND_PARTY_LEAVE_SUCCESS))
                            .replace("{playername}", member.getName())
                            .replace("{player}", member.getDisplayName()));
                }
    ...
    public void disband(Player owner) {
        ...
        for (Player p : pa.members) {
            p.sendMessage(SupportPAPI.getSupportPAPI().replace(p, Language.getMsg(p, Messages.COMMAND_PARTY_DISBAND_SUCCESS)));
        }
        pa.members.clear();
    ...
    public void removePlayer(Player owner, Player target) {
        ...
        if (p != null) {
            if (p.members.contains(target)) {
                for (Player mem : p.members) {
                    mem.sendMessage(SupportPAPI.getSupportPAPI().replace(mem, Language.getMsg(mem, Messages.COMMAND_PARTY_REMOVE_SUCCESS))
