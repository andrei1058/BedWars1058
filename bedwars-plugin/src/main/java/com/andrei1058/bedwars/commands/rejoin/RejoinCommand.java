...
public class RejoinCommand extends BukkitCommand {
    ...
    public boolean execute(CommandSender s, String st, String[] args) {
        ...
        if (!p.hasPermission(Permissions.PERMISSION_REJOIN)) {
            p.sendMessage(langService.getMsg(p, Messages.COMMAND_NOT_FOUND_OR_INSUFF_PERMS));
            return true;
        }
        ...
        if (rj == null) {
            p.sendMessage(langService.getMsg(p, Messages.REJOIN_NO_ARENA));
            Sounds.playSound("rejoin-denied", p);
            return true;
        }

        if (!rj.canReJoin()) {
            p.sendMessage(langService.getMsg(p, Messages.REJOIN_DENIED));
            Sounds.playSound("rejoin-denied", p);
            return true;
        }

        p.sendMessage(langService.getMsg(p, Messages.REJOIN_ALLOWED).replace("{arena}", rj.getArena().getDisplayName()));
