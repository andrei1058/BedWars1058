...
public class CmdStart extends SubCommand {
    ...
    public boolean execute(String[] args, CommandSender s) {
        ...
        if (a == null){
            p.sendMessage(SupportPAPI.getSupportPAPI().replace(p, Language.getMsg(p, Messages.COMMAND_FORCESTART_NOT_IN_GAME)));
            return true;
        }
        if (!a.isPlayer(p)){
            p.sendMessage(SupportPAPI.getSupportPAPI().replace(p, Language.getMsg(p, Messages.COMMAND_FORCESTART_NOT_IN_GAME)));
        ...
        if (!(p.hasPermission(Permissions.PERMISSION_ALL) || p.hasPermission(Permissions.PERMISSION_FORCESTART))){
            p.sendMessage(SupportPAPI.getSupportPAPI().replace(p, Language.getMsg(p, Messages.COMMAND_FORCESTART_NO_PERM)));
            return true;
        }
        ...
        p.sendMessage(SupportPAPI.getSupportPAPI().replace(p, Language.getMsg(p, Messages.COMMAND_FORCESTART_SUCCESS)));
        return true;
    }
