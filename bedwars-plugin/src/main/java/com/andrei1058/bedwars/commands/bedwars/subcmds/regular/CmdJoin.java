...
public class CmdJoin extends SubCommand {
    ...
    public boolean execute(String[] args, CommandSender s) {
        ...
        Player p = (Player) s;
        if (args.length < 1) {
            s.sendMessage(getLangService().getMsg(p, Messages.COMMAND_JOIN_USAGE));
            return true;
        }
        if (args[0].equalsIgnoreCase("random")){
            if (!Arena.joinRandomArena(p)){
                s.sendMessage(getLangService().getMsg(p, Messages.COMMAND_JOIN_NO_EMPTY_FOUND));
        ...
        s.sendMessage(getLangService().getMsg(p, Messages.COMMAND_JOIN_GROUP_OR_ARENA_NOT_FOUND).replace("{name}", args[0]));
        return true;
    }
