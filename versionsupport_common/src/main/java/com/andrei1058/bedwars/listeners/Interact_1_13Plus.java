...
public class Interact_1_13Plus implements Listener {
    ...
    public void onInventoryInteract(PlayerInteractEvent e) {
        ...
        if (b == null) return;
        if (b.getWorld().getName().equals(VersionCommon.api.getLobbyWorld()) || VersionCommon.api.getArenaUtil().getArenaByPlayer(e.getPlayer()) != null) {
            switch (b.getType().toString()) {
                ...
                case "DAMAGED_ANVIL":
                    if (VersionCommon.api.getConfigs().getMainConfig().getBoolean(ConfigPath.GENERAL_CONFIGURATION_DISABLE_ANVIL)) {
                        ...
                        if (VersionCommon.api.getArenaUtil().isSpectating(e.getPlayer())) e.setCancelled(true);
                    }
                    break;
            }
        }
