...
public class v1_14_R1 extends VersionSupport {
    ...
    public List<Block> calculateExplosionBlocks(IArena arena, Entity source, Location explosionLocation, int radius, boolean fire, BiFunction<Location, Block, Boolean> callback) {
        ...
        net.minecraft.server.v1_14_R1.Entity sourceEntity = null;
        if (source != null) {
            sourceEntity = ((CraftEntity) source).getHandle();

        }

        net.minecraft.server.v1_14_R1.World world = ((CraftWorld) Objects.requireNonNull(bukkitWorld)).getHandle();

        double locX = explosionLocation.getX();
        double locY = explosionLocation.getY();

        if (sourceEntity instanceof EntityTNTPrimed) {
            locY += sourceEntity.getHeight() / 16.0F;
        }

        double locZ = explosionLocation.getZ();

        Explosion explosion = new Explosion(world, sourceEntity, locX, locY, locZ, radius, fire, Explosion.Effect.BREAK);
                            ...
                            if (!iblockdata.isAir() || !fluid.isEmpty()) {
                                ...
                                if (allow) {
                                    ...
                                    if (sourceEntity != null) {
                                        f2 = sourceEntity.a(explosion, world, blockposition, iblockdata, fluid, f2);
                                    }
