package com.andrei1058.bedwars.arena.mapreset.eswm;
...
public class ESlimeAdapter extends RestoreAdapter {
    ...
    public ESlimeAdapter(Plugin plugin) {
        super(plugin);
        slime = (SlimePlugin) Bukkit.getPluginManager().getPlugin("Enhanced-SlimeWorldManager");
    ...
    public void onEnable(@NotNull IArena a) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(getOwner(), () -> {
            ...
            SlimeLoader flat = slime.getLoader(SlimeLoader.FILE);
            String[] spawn = a.getConfig().getString("waiting.Loc").split(",");

            SlimePropertyMap spm = new SlimePropertyMap();
            spm.set(SlimeProperties.SPAWN_Y, 3);
            try {
                spm.set(SlimeProperties.SPAWN_X, (int) Double.parseDouble(spawn[0]));
                spm.set(SlimeProperties.SPAWN_Y, (int) Double.parseDouble(spawn[1]));
                spm.set(SlimeProperties.SPAWN_Z, (int) Double.parseDouble(spawn[2]));
            } catch (NumberFormatException ex) {
                spm.set(SlimeProperties.SPAWN_X, 0);
                spm.set(SlimeProperties.SPAWN_Y, 1);
                spm.set(SlimeProperties.SPAWN_Z, 0);
            }
            spm.set(SlimeProperties.ALLOW_ANIMALS, false);
            spm.set(SlimeProperties.ALLOW_MONSTERS, false);
            spm.set(SlimeProperties.DIFFICULTY, "easy");
            spm.set(SlimeProperties.PVP, true);
    ...
    public void onSetupSessionStart(ISetupSession s) {
        Bukkit.getScheduler().runTaskAsynchronously(getOwner(), () -> {
            SlimeLoader sLoader = slime.getLoader(SlimeLoader.FILE);
            ...
            try {
                ...
                if (sLoader.worldExists(s.getWorldName())) {
                    ...
                    Bukkit.getScheduler().runTask(getOwner(), () -> s.getPlayer().sendMessage(ChatColor.GREEN + "Loading world from Enhanced-SlimeWorldManager container."));
                } else {
                    if (new File(Bukkit.getWorldContainer(), s.getWorldName() + "/level.dat").exists()) {
                        Bukkit.getScheduler().runTask(getOwner(), () -> s.getPlayer().sendMessage(ChatColor.GREEN + "Importing world to the Enhanced-SlimeWorldManager container."));
    ...
    public boolean isWorld(String name) {
        try {
            return slime.getLoader(SlimeLoader.FILE).worldExists(name);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return false;
    ...
    public void deleteWorld(String name) {
        Bukkit.getScheduler().runTaskAsynchronously(getOwner(), () -> {
            try {
                slime.getLoader(SlimeLoader.FILE).deleteWorld(name);
    ...
    public void cloneArena(String name1, String name2) {
        Bukkit.getScheduler().runTaskAsynchronously(getOwner(), () -> {
            ...
            spm.set(SlimeProperties.SPAWN_Y, 3);
            spm.set(SlimeProperties.SPAWN_X, 0);
            spm.set(SlimeProperties.SPAWN_Y, 10);
            spm.set(SlimeProperties.SPAWN_Z, 0);
            spm.set(SlimeProperties.ALLOW_ANIMALS, false);
            spm.set(SlimeProperties.ALLOW_MONSTERS, false);
            spm.set(SlimeProperties.DIFFICULTY, "easy");
            ...
            try {
                // Note that this method should be called asynchronously
                SlimeWorld world = slime.loadWorld(slime.getLoader(SlimeLoader.FILE), name1, true, spm);
                world.clone(name2, slime.getLoader(SlimeLoader.FILE));
    ...
    public List<String> getWorldsList() {
        try {
            return slime.getLoader(SlimeLoader.FILE).listWorlds();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void convertWorlds() {
        ...
        File ff;
        SlimeLoader sl = slime.getLoader(SlimeLoader.FILE);
        if (dir.exists()) {
